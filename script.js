...
function renderizarHistorico() {
    ...
    const filtroEquip = document.getElementById('filtroEquipamento').value;
    const filtroData = document.getElementById('filtroDataEstado').value;

    // Obter data de hoje no formato YYYY-MM-DD para comparação exata
    const hoje = new Date();
    ...
    let filtradas = [...reservasAtivas];

    // 1. Filtrar por Equipamento
    if (filtroEquip !== "Todos") {
        filtradas = filtradas.filter(res => res.equipamento === filtroEquip);
    }

    // 2. Filtrar por Data (Realizados vs Não Realizados)
    if (filtroData === "Realizados") {
        // Eventos que aconteceram antes de hoje
        filtradas = filtradas.filter(res => res.data < dataHojeString);
    } else if (filtroData === "NaoRealizados") {
        // Eventos de hoje ou futuros
        filtradas = filtradas.filter(res => res.data >= dataHojeString);
    }
    ...
    if (filtradas.length === 0) {
        corpo.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:20px;">Nenhuma reserva encontrada para os filtros selecionados.</td></tr>`;
        return;
    }
